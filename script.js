...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function getRandomPlant() {
        ...
        // Choose selection strategy based on cache size
        // If we have many cached plants (>=10), prefer cached to avoid API calls
        // If we have few cached plants (<10), use all curated IDs to ensure variety
        let randomId;
        const MIN_CACHED_FOR_EXCLUSIVE = 10;
        if (cachedIds.length >= MIN_CACHED_FOR_EXCLUSIVE) {
            // Many cached plants available - prefer cached to reduce API calls
            ...
            // Few cached plants - use all curated IDs to ensure variety
            randomId = CURATED_PLANT_IDS[Math.floor(Math.random() * CURATED_PLANT_IDS.length)];
            console.log(`🎲 getRandomPlant: Using curated plant ID ${randomId} (only ${cachedIds.length} cached plants, using full list for variety)`);
        }
        ...
        try {
            ...
            // If fetching failed and we have cached plants available, try using a cached plant
            if (cachedIds.length > 0) {
                console.log(`🎲 getRandomPlant: API call failed, trying cached plant (${cachedIds.length} available)...`);
                // Try a random cached plant
                const fallbackId = cachedIds[Math.floor(Math.random() * cachedIds.length)];
                const cacheKey = `plant_${fallbackId}`;
                const cached = localStorage.getItem(cacheKey);
                if (cached) {
                    try {
                        const plantData = JSON.parse(cached);
                        console.log(`🎲 getRandomPlant: ✅ Using cached plant ID ${fallbackId} as fallback`);
                        return plantData;
                    } catch (parseError) {
                        console.warn(`Failed to parse cached plant ${fallbackId}:`, parseError);
                    }
                }
            }
